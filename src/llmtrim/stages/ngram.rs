...
//!
//! Mining (see `crate::llmtrim::stages::ngram_sa`): the repeated phrases are the *maximal
...
impl Transform for NgramStage {
    ...
    fn apply(
        ...
        let prose: Vec<usize> = (0..segs.len())
            .filter(|&i| !crate::llmtrim::stages::tools::is_structured_segment(&segs[i].1))
            .collect();
        if prose.is_empty() {
            return Ok(());
        }
...
mod tests {
    ...
    fn toks(counter: &dyn crate::llmtrim::tokenizer::TokenCounter, s: &str) -> usize {
        counter.count(s)
    }
