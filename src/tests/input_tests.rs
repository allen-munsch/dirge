...
fn ctrl_r_enters_search_mode() {
    ...
    // most recent history entry shown
...
fn ctrl_r_with_empty_history_is_noop() {
    ...
    // no history → search can't activate
    assert!(!editor.is_in_search());
    assert_eq!(editor.buffer.as_str(), "hello");
}
...
fn search_typing_narrows_query_and_updates_match() {
    ...
    editor.handle_key(ctrl(KeyCode::Char('r')));
    // default: most recent = "different"
    assert_eq!(editor.search_match_text(), "different");

    // type 's' → should match "second" (newest match for "s")
...
fn search_backspace_narrows_query() {
    ...
    // backspace to "ab" → newest match for "ab"
    editor.handle_key(press(KeyCode::Backspace));
    assert_eq!(editor.search_query(), "ab");
    // both "abc" and "abd" match "ab", newest first = "abd"
...
fn ctrl_r_again_cycles_to_older_match() {
    ...
    // Ctrl+R again → wrap around to newest
    editor.handle_key(ctrl(KeyCode::Char('r')));
    assert_eq!(editor.search_match_text(), "third foo");
}
...
fn search_enter_accepts_match() {
    ...
    // Enter accepts: buffer becomes the match, search mode exits
    editor.handle_key(press(KeyCode::Enter));
    assert!(!editor.is_in_search());
...
fn search_esc_cancels_and_restores_draft() {
    ...
    // Esc cancels: buffer restored to draft, search mode exits
...
#[test]
fn typing_in_search_then_enter_returns_to_normal_mode() {
    let mut editor = InputEditor::new();
    type_str(&mut editor, "alpha");
    editor.handle_key(press(KeyCode::Enter));
    type_str(&mut editor, "bravo");
    editor.handle_key(press(KeyCode::Enter));

    // enter search, accept match — typing 'p' should uniquely match "alpha"
    editor.handle_key(ctrl(KeyCode::Char('r')));
    editor.handle_key(press(KeyCode::Char('p')));
    assert!(editor.is_in_search());
    editor.handle_key(press(KeyCode::Enter));

    // should be back to normal: typing works normally
    assert!(!editor.is_in_search());
    type_str(&mut editor, " extra");
    assert_eq!(editor.buffer.as_str(), "alpha extra");
}

#[test]
fn search_find_behavior() {
    let mut editor = InputEditor::new();
    type_str(&mut editor, "alpha");
    editor.handle_key(press(KeyCode::Enter));
    type_str(&mut editor, "bravo");
    ...
    // Enter search, type 'a' — should match "alpha" (newest match containing 'a')
    editor.handle_key(ctrl(KeyCode::Char('r')));
    assert_eq!(editor.search_match_text(), "bravo");
    ...
    editor.handle_key(ctrl(KeyCode::Char('r')));
    assert_eq!(editor.search_match_text(), "alpha");
    assert_eq!(editor.buffer.as_str(), "alpha");
    // type 'l' to narrow — only "alpha" contains "al"
    editor.handle_key(press(KeyCode::Char('l')));
    assert_eq!(editor.search_match_text(), "alpha");
}

#[test]
fn cancel_search_restores_draft() {
    let mut editor = InputEditor::new();
    type_str(&mut editor, "history entry");
    editor.handle_key(press(KeyCode::Enter));
    ...
    // calls cancel_search() externally rather than dispatching through handle_key
...
fn load_history_populates_for_ctrl_r() {
    ...
    // Enter search — should see "third message" (newest)
    editor.handle_key(ctrl(KeyCode::Char('r')));
    assert!(editor.is_in_search());
...
fn load_history_skips_dupes_and_empty() {
    ...
    // Filter for "msg3" which doesn't exist
